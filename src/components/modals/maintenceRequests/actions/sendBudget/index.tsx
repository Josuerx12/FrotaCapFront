...
import { FaCar, FaFileArchive, FaSpinner, FaTimes } from "react-icons/fa";
...
const SendBudgetModal = ({ show, handleClose, request }: Props) => {
  const {
    register,
    reset: resetForm,
    handleSubmit,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center justify-center gap-4 mt-4">
          ...
          >
            {isLoading ? (
              <>
                <FaSpinner className="animate-spin" /> Enviando orçamento
              </>
            ) : (
              <>
                <FaFileArchive /> Enviar Orçamento
              </>
            )}
          </button>
        </div>
